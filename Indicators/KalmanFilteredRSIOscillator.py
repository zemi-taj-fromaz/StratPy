...
class KalmanFilteredRsiOscillator:
    ...
    def run_test(self):
        ...
        for N in range(2, 13, 2):
            ...
            self.stateEstimate[:] = self.timeseries["close"][:N]
            self.errorCovariance = np.zeros(N)   # Update with values from timeseries
            self.errorCovariance[:] = 1.0  # Update all elements to 1.0
            for measurementNoise in [x * 0.1 for x in range(5, 75, 5)]:  # Step by 0.1
                for rsiPeriod in range(2, 13, 2):
    ...
    def calculate(self,  processNoise, measurementNoise, N, rsiPeriod):
        ...
        lowest = rsi.rolling(100).min()
        highest = rsi.rolling(100).max()

        ewmaOsc = (rsi - lowest) / (highest - lowest) - 0.5
