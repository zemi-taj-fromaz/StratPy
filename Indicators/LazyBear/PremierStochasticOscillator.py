...
class Pso:
    ...
    def store_result(self,equity, stochlen , smoothlen ):
        """
        Store the result in the heap, keeping only the top 10 results.
        """
        heapq.heappush(self.top_results, (equity,  stochlen , smoothlen ))
    ...
    def run_test(self):
        ...
        for stochlen in range(15, 28):
            for smoothlen in range(15, 30, 1):
                equity = self.calculate( stochlen , smoothlen )
                print(equity)
                self.store_result(equity,stochlen , smoothlen )

        self.print_top_results()

    def calculate(self,   stochlen , smoothlen):
