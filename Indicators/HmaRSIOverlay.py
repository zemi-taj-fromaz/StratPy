...
class HmaRSIOverlay:
    ...
    def run_test(self):
        ...
        """
        for demaLength in range(9, 25, 1):
            for rsi_length in range(4, 12, 1):
                for long_threshold in range(55, 69, 1):
                    for short_threshold in range(48, 58, 1):
    ...
    def calculate(self, dema_length: int, rsi_length: int, long_threshold: float, short_threshold: float):
        ...
        # Calculate DEMA
        self.timeseries['DEMA'] = ta.hma(self.timeseries['close'], length=dema_length)
